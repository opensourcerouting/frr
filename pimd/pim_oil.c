...
char *pim_channel_oil_dump(struct channel_oil *c_oil, char *buf, size_t size)
{
	...
	sg.src = c_oil->source;
	sg.grp = c_oil->group;
	ifp = pim_if_find_by_vif_index(c_oil->pim, c_oil->iif.ifindex);
	snprintfrr(buf, size, "%pSG IIF: %s, OIFS: ", &sg,
		   ifp ? ifp->name : "(?)");

	out = buf + strlen(buf);
	frr_each (channel_oif_list, &c_oil->oif_list, oif) {
		ifp = pim_if_find_by_vif_index(c_oil->pim, oif->ifindex);
...
void pim_clear_nocache_state(struct pim_interface *pim_ifp)
{
	...
	frr_each_safe (rb_pim_oil, &pim_ifp->pim->channel_oil_head, c_oil) {
		...
		if (c_oil->iif.ifindex != pim_ifp->mroute_vif_index)
			continue;
...
{
	if (PIM_DEBUG_MROUTE) {
		pim_sgaddr sg = {.src = c_oil->source,
				 .grp = c_oil->group};
			...
			uint32_t proto_mask, const char *caller)
{
	 ...
	 */
	if (coif == NULL || !CHECK_FLAG(coif->flags, proto_mask)) {
		if (PIM_DEBUG_MROUTE)
	...
	channel_oil_oif_delete(channel_oil, pim_ifp->mroute_vif_index, proto_mask);
	...
	if (PIM_DEBUG_MROUTE) {
		struct interface *iifp = pim_if_find_by_vif_index(pim_ifp->pim,
								  channel_oil->iif.ifindex);

		zlog_debug("%s(%s): (S,G)=(%pPAs,%pPAs): proto_mask=%u IIF:%s OIF=%s vif_index=%d",
			   __func__, caller, &channel_oil->source,
			   &channel_oil->group, proto_mask,
			...
			uint32_t proto_mask, const char *caller)
{
	...
	if (!pim_addr_is_any(channel_oil->source) &&
	    ...
	    channel_oil->iif.ifindex < MAXVIFS &&
	    pim_ifp->mroute_vif_index == channel_oil->iif.ifindex &&
	...
	if (coif && CHECK_FLAG(coif->flags, proto_mask)) {
		coif = channel_oil_oif_add(channel_oil, pim_ifp->mroute_vif_index, proto_mask);

		if (PIM_DEBUG_MROUTE) {
			zlog_debug(
				"%s %s: existing protocol mask %u requested OIF %s (vif_index=%d) for channel (S,G)=(%pPAs,%pPAs)",
	 ...
	 */
	if (channel_oil->iif.ifindex != MAXVIFS) {
		if (pim_upstream_mroute_add(channel_oil, __func__)) {
			if (PIM_DEBUG_MROUTE) {
				zlog_debug(
			...
			if (coif)
				channel_oil_oif_add(channel_oil, pim_ifp->mroute_vif_index, 0);
			else
				channel_oil_oif_delete(channel_oil, pim_ifp->mroute_vif_index, 0);
			return -5;
		}
	}
...
int pim_channel_oil_empty(struct channel_oil *c_oil)
{
	...
	return !channel_oif_list_count(&c_oil->oif_list);
}
...
void channel_oil_to_mfcc(struct channel_oil *oil, struct mfcctl *mfcc)
{
	...
	mfcc->mfcc_parent = oil->iif.ifindex;
	frr_each (channel_oif_list, &oil->oif_list, oif) {
		if (oif->ifindex == oil->iif.ifindex &&
		    !pim_mroute_allow_iif_in_oil(oil, oif->ifindex))
		    continue;
		if (CHECK_FLAG(oif->flags, PIM_OIF_FLAG_MUTE))
			continue;
		
		mfcc->mfcc_ttls[oif->ifindex] = 1;
	}
}
#else
void channel_oil_to_mfcc(struct channel_oil *oil, struct mf6cctl *mf6cc)
{
	...
	mf6cc->mf6cc_parent = oil->iif.ifindex;
	frr_each (channel_oif_list, &oil->oif_list, oif) {
		if (oif->ifindex == oil->iif.ifindex &&
		    !pim_mroute_allow_iif_in_oil(oil, oif->ifindex))
		    continue;
		...
		IF_SET(oif->ifindex, &mf6cc->mf6cc_ifset);
	}
}
#endif
