...
int pim_mroute_msg_nocache(int fd, struct interface *ifp, const kernmsg *msg)
{
	...
	if (dodense) {
		...
		*/
		...
		if (up->rpf.source_nexthop.interface &&
		    up->channel_oil->iif.ifindex >= MAXVIFS) {
	...
	/* resolve mfcc_parent prior to mroute_add in channel_add_oif */
	if (up->rpf.source_nexthop.interface && up->channel_oil->iif.ifindex >= MAXVIFS) {
		pim_upstream_mroute_iif_update(up->channel_oil, __func__);
	}
...
static int pim_mroute_wrongvif_prefer_ingress(struct interface *ifp, pim_sgaddr *sg)
{
	...
	if (up->channel_oil->installed && up->rpf.source_nexthop.interface == ifp &&
	    up->channel_oil->iif.ifindex == ingress_vif)
		/*
...
static int pim_mroute_add(struct channel_oil *c_oil, const char *name)
{
...
#if PIM_IPV == 4
		mfcc.mfcc_ttls[c_oil->iif.ifindex] = 1;
#else
		IF_SET(c_oil->iif.ifindex, &mfcc.mf6cc_ifset);
#endif
	}
	 ...
	 */
	if (!c_oil->installed && !pim_addr_is_any(c_oil->source) &&
	    c_oil->iif.ifindex != 0) {
#if PIM_IPV == 4
		mfcc.mfcc_parent = 0;
#else
	...
	if (!err && !c_oil->installed && !pim_addr_is_any(c_oil->source) &&
	    c_oil->iif.ifindex != 0) {
#if PIM_IPV == 4
		mfcc.mfcc_parent = c_oil->iif.ifindex;
#else
		mfcc.mf6cc_parent = c_oil->iif.ifindex;
#endif
		frr_with_privs (&pimd_privs)
...
int pim_upstream_mroute_add(struct channel_oil *c_oil, const char *name)
{
	...
	if (c_oil->iif.ifindex != iif) {
		c_oil->iif.ifindex = iif;
...
void pim_static_mroute_iif_update(struct channel_oil *c_oil,
				...
				const char *name)
{
	if (c_oil->iif.ifindex == input_vif_index)
		return;

	c_oil->iif.ifindex = input_vif_index;
	...
	if (!c_oil->installed) {
		if (PIM_DEBUG_MROUTE) {
			...
			struct interface *iifp =
				pim_if_find_by_vif_index(pim, c_oil->iif.ifindex);
