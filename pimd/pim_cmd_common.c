		    ...
		    json_object *json)
{
	...
	frr_each (rb_pim_oil, &pim->channel_oil_head, c_oil) {
		...
		ifp_in = pim_if_find_by_vif_index(pim, c_oil->iif.ifindex);
		...
		frr_each (channel_oif_list, &c_oil->oif_list, oif) {
			...
			ifp_out = pim_if_find_by_vif_index(pim, oif->ifindex);
			...
			if (json) {
				...
				snprintf(flag, sizeof(flag), "(%c%c%c%c%c)",
					 (oif->flags & PIM_OIF_FLAG_PROTO_GM) ? 'I' : ' ',
					 (oif->flags & PIM_OIF_FLAG_PROTO_PIM) ? 'J' : ' ',
					 (oif->flags & PIM_OIF_FLAG_PROTO_VXLAN) ? 'V' : ' ',
					 (oif->flags & PIM_OIF_FLAG_PROTO_STAR) ? '*' : ' ',
					 (oif->flags & PIM_OIF_FLAG_MUTE) ? 'M' : ' ');
		 ...
		 bool fill, json_object *json)
{
	...
	frr_each (rb_pim_oil, &pim->channel_oil_head, c_oil) {
		...
		frr_each (channel_oif_list, &c_oil->oif_list, oif) {
			...
			if (oif->flags & PIM_OIF_FLAG_MUTE)
				continue;

			if (c_oil->iif.ifindex == oif->ifindex &&
			    !pim_mroute_allow_iif_in_oil(c_oil, oif->ifindex))
				continue;
			...
			if (json) {
				...
				json_object_string_add(json_ifp_out,
						       "inboundInterface",
						       in_ifname);
				json_object_int_add(json_ifp_out, "iVifI", c_oil->iif.ifindex);
				json_object_string_add(json_ifp_out, "outboundInterface",
						       out_ifname);
				json_object_int_add(json_ifp_out, "oVifI",
						    oif->ifindex);
				json_object_int_add(json_ifp_out, "ttl", 1);
	...
	for (ALL_LIST_ELEMENTS_RO(pim->static_routes, node, s_route)) {
		...
		frr_each (channel_oif_list, &s_route->c_oil.oif_list, oif) {
			...
			ttl = s_route->oif_ttls[oif->ifindex];
			if (ttl < 1)
				continue;
