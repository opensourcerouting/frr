				      ...
				      bool i_am_rp)
{
	...
	if (PIM_DEBUG_MROUTE)
		zlog_debug("%s: SG(%pPA, %pPA) IAmRP iif:%d RPIF:%d upstream:%s", __func__,
			   &oil->source, &oil->group, oil->iif.index, rp_if,
			   upstream ? "yes" : "no");
...
static void pimsb_mroute_do(struct channel_oil *oil, bool install)
{
	...
	if (upstream) {
		i_am_lhr = CHECK_FLAG(upstream->flags, (PIM_UPSTREAM_FLAG_MASK_SRC_LHR |
	...
	if (!is_static && channel_oif_list_count(&oil->oif_list) == 0)
...
static void pimsb_client_data_start(const struct mroute_event *me)
{
	...
	if (PIM_DEBUG_MROUTE)
		zlog_debug("%s: unconnected source (%s, %pPA) %s", __func__, ifp->name, &sg_p.src,
			   up ? CHECK_FLAG(up->flags, PIM_UPSTREAM_FLAG_MASK_SPT_DESIRED)
					   ? "upstream switch"
					   : "upstream no switch"
			      : "no upstream");

	if (up && CHECK_FLAG(up->flags, PIM_UPSTREAM_FLAG_MASK_SPT_DESIRED)) {
...
static void pimsb_client_data_stop(const struct mroute_event *me)
{
	...
	if (CHECK_FLAG(up->flags, PIM_UPSTREAM_FLAG_MASK_SRC_IGMP)) {
...
static void pimsb_client_spt_join(const struct mroute_event *me)
{
	 ...
	 * we leak later)
	 */
	if (CHECK_FLAG(up->flags,
		       (PIM_UPSTREAM_FLAG_MASK_SPT_DESIRED | PIM_UPSTREAM_FLAG_MASK_SRC_LHR))) {
