			...
			struct bgp *bgp_nexthop, struct bgp_path_info *source_pi)
{
	...
	case AFI_IP6:
		...
		if (bgp_attr_get_srv6_l3service(pi->attr)) {
			...
			tmp_prefix.prefix = bgp_attr_get_srv6_l3service(pi->attr)->sid;
		...
		if (local_sid == false && bgp_attr_get_srv6_l3service(pi->attr)) {
			p->prefixlen = IPV6_MAX_BITLEN;
			if (bgp_attr_get_srv6_l3service(pi->attr)->transposition_len != 0 &&
			    BGP_PATH_INFO_NUM_LABELS(pi)) {
				IPV6_ADDR_COPY(&p->u.prefix6, &bgp_attr_get_srv6_l3service(pi->attr)->sid);
				transpose_sid(&p->u.prefix6,
					      decode_label(&pi->extra->labels->label[0]),
					      bgp_attr_get_srv6_l3service(pi->attr)->transposition_offset,
					      bgp_attr_get_srv6_l3service(pi->attr)->transposition_len,
