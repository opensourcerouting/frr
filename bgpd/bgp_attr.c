...
static const struct message attr_str[] = { { BGP_ATTR_ORIGIN, "ORIGIN" },
...
void bgp_nhc_tlv_add(struct attr *attr, struct bgp_nhc_tlv *tlv)
{
...
static uint32_t bgp_nhc_hash_key_make(const void *p)
{
	...
	key = jhash(&nhc->afi, 2, key);
	key = jhash(&nhc->safi, 1, key);
	key = jhash(&nhc->nh_length, 1, key);
	key = jhash(&nhc->nh_ipv4, 4, key);
	key = jhash(&nhc->nh_ipv6_global, 16, key);
	key = jhash(&nhc->nh_ipv6_local, 16, key);
	if (nhc->tlvs)
...
static bool bgp_nhc_hash_cmp(const void *p1, const void *p2)
{
	...
	return (nhc1->afi == nhc2->afi && nhc1->safi == nhc2->safi &&
		...
		IPV6_ADDR_CMP(&nhc1->nh_ipv6_global, &nhc2->nh_ipv6_global) &&
		IPV6_ADDR_CMP(&nhc1->nh_ipv6_local, &nhc2->nh_ipv6_local) &&
...
static void bgp_nhc_finish(void)
{
...
#define MIX3(a, b, c)	key = jhash_3words((a), (b), (c), key)
	...
	if (attr->nhc)
		MIX(bgp_nhc_hash_key_make(attr->nhc));
...
const uint8_t attr_flags_values[] = {
	...
	[BGP_ATTR_IPV6_EXT_COMMUNITIES] = BGP_ATTR_FLAG_OPTIONAL | BGP_ATTR_FLAG_TRANS,
...
static int bgp_attr_nhc(struct bgp_attr_parser_args *args)
{
	...
	if (length < BGP_NHC_MIN_LEN) {
		zlog_err("%pBP sent BGP Next Hop Dependent Characteristics attribute length is too short: %d",
			 peer, length);
	...
	if (nhc->nh_length == BGP_ATTR_NHLEN_IPV4) {
		...
		stream_get(&nhc->nh_ipv6_global, s, IPV6_MAX_BYTELEN);
		length -= IPV6_MAX_BYTELEN;
		if (IN6_IS_ADDR_LINKLOCAL(&nhc->nh_ipv6_global)) {
			if (!peer->nexthop.ifp) {
	 ...
	 * +-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+
	 */
	/* Need to handle this loop differently according to nh_length, because BGP_NHC_TLV_MIN_LEN 
	 * might be 16, not 4 
	 */
	while (length && STREAM_READABLE(s) >= BGP_NHC_TLV_MIN_LEN) {
		...
		if (tlv->code == BGP_ATTR_NHC_TLV_NNHN) {
			...
			for (int i = 0; i < len; i += IPV4_MAX_BYTELEN)
				zlog_info("ROUTER-ID %d: %pI4", i, &tlv->value[i]);
		}

		if (bgp_nhc_tlv_find(attr, tlv_code))
			bgp_nhc_tlv_free(tlv);

		bgp_nhc_tlv_add(attr, tlv);
...
void bgp_packet_nhc(struct stream *s, struct peer *peer, afi_t afi, safi_t safi, struct attr *attr)
{
	size_t sizep;
	iana_afi_t pkt_afi = IANA_AFI_IPV4;
	iana_safi_t pkt_safi = IANA_SAFI_UNICAST;
	...
	/* Put TLVs */
	for (tlv = attr->nhc->tlvs; tlv; tlv = tlv->next) {
		stream_putw(s, tlv->code);
		stream_putw(s, tlv->length);
		stream_put(s, tlv->value, tlv->length);
		if (bgp_debug_update(peer, NULL, NULL, 0))
			zlog_debug("%pBP: sending NHC TLV code: %u, length: %u, value: %p", peer,
				   tlv->code, tlv->length, tlv->value);
	}
				...
				bool addpath_capable, uint32_t addpath_tx_id)
{
	...
	if (CHECK_FLAG(attr->flag, ATTR_FLAG_BIT(BGP_ATTR_NHC)))
		bgp_packet_nhc(s, peer, afi, safi, attr);
