	    ...
	    struct prefix *nexthop_orig, int nexthop_self_flag, int debug)
{
	...
	for (bpi = bgp_dest_get_bgp_path_info(bn); bpi; bpi = bpi->next) {
		if (!bpi->extra || !bpi->extra->vrfleak ||
			...
			continue;
		if (bgp_attr_get_srv6_l3service(new_attr) && !bgp_attr_get_srv6_l3service(bpi->attr) &&
					...
					mpls_label_t *label)
{
	...
	if (from_bgp->vpn_policy[afi].tovpn_sid_locator && from_bgp->vpn_policy[afi].tovpn_sid) {
		...
		encode_label(from_bgp->vpn_policy[afi].tovpn_sid_transpose_label, label);
		{
			...
			if (CHECK_FLAG(from_bgp->vpn_policy[afi].flags,
				...
				l3->func_len = BGP_PREFIX_SID_SRV6_MAX_FUNCTION_LENGTH_FOR_BGP;
			else
				l3->func_len =
			...
			if (l3->func_len > BGP_PREFIX_SID_SRV6_MAX_FUNCTION_LENGTH_FOR_LABEL) {
				l3->transposition_len = 0;
				l3->transposition_offset = 0;
				IPV6_ADDR_COPY(&l3->sid,
					       from_bgp->vpn_policy[afi].tovpn_sid);
			} else {
				l3->transposition_len =
					from_bgp->vpn_policy[afi].tovpn_sid_locator->function_bits_length;
				l3->transposition_offset =
					from_bgp->vpn_policy[afi].tovpn_sid_locator->block_bits_length +
					from_bgp->vpn_policy[afi].tovpn_sid_locator->node_bits_length;
				IPV6_ADDR_COPY(&l3->sid,
	...
	if (from_bgp->tovpn_sid_locator && from_bgp->tovpn_sid) {
		...
		encode_label(from_bgp->tovpn_sid_transpose_label, label);
		{
			...
			l3->loc_block_len = from_bgp->tovpn_sid_locator->block_bits_length;
			l3->loc_node_len = from_bgp->tovpn_sid_locator->node_bits_length;
			if (CHECK_FLAG(from_bgp->vrf_flags, BGP_VRF_TOVPN_SID_FUNC_WIDE))
				l3->func_len = BGP_PREFIX_SID_SRV6_MAX_FUNCTION_LENGTH_FOR_BGP;
			else
				l3->func_len = from_bgp->tovpn_sid_locator->function_bits_length;
			l3->arg_len = from_bgp->tovpn_sid_locator->argument_bits_length;
			if (l3->func_len > BGP_PREFIX_SID_SRV6_MAX_FUNCTION_LENGTH_FOR_LABEL) {
				l3->transposition_len = 0;
				l3->transposition_offset = 0;
				IPV6_ADDR_COPY(&l3->sid, from_bgp->tovpn_sid);
			} else {
				l3->transposition_len =
				...
				struct bgp_path_info *path_vrf) /* route */
{
	...
	while (bpi_next) {
		...
		if (!bpi->extra || !bpi->extra->vrfleak || bpi->extra->vrfleak->parent != path_vrf)
			continue;
		if (bgp_attr_get_srv6_l3service(path_vrf->attr) && !bgp_attr_get_srv6_l3service(bpi->attr) &&
		    !bgp_labels_is_implicit_null(bpi))
			/* SRv6 path can not overwrite MPLS path */
			continue;
		if ((!bgp_attr_get_srv6_l3service(path_vrf->attr) && BGP_PATH_INFO_NUM_LABELS(path_vrf) == 1 &&
	...
	if (aspath_loop_check(path_vpn->attr->aspath, to_bgp->as) > aspath_loop_count) {
		for (bpi = bgp_dest_get_bgp_path_info(bn); bpi;
			...
			if (!bpi->extra || !bpi->extra->vrfleak ||
				...
				continue;
			if (bgp_attr_get_srv6_l3service(path_vpn->attr) && !bgp_attr_get_srv6_l3service(bpi->attr) &&
			    !bgp_labels_is_implicit_null(bpi))
				/* SRv6 path can not overwrite MPLS path */
				continue;
