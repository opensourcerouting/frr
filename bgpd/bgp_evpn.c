				   ...
				   safi_t src_safi)
{
	...
	if (src_afi == AFI_IP6 &&
		...
		if (src_attr && src_attr->nexthop.s_addr != 0) {
			...
			bgp_attr_set_evpn_overlay(&attr, bre);
			zlog_info("BLET: 2 %p", attr.evpn_overlay);
		}
	}
					   ...
					   struct bgp_path_info *parent_pi)
{
	...
	if (bre) {
		zlog_info("BLET %s 1", __func__);
		if (bre->type != OVERLAY_INDEX_GATEWAY_IP) {
			zlog_info("BLET %s 2", __func__);
			if (afi == AFI_IP6)
