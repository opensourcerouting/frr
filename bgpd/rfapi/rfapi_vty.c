			   ...
			   struct bgp_path_info *bpi, safi_t safi)
{
	...
	if (bgp_attr_get_srv6_l3service(bpi->attr) || bgp_attr_get_srv6_vpn(bpi->attr)) {
		struct in6_addr *sid_tmp = bgp_attr_get_srv6_l3service(bpi->attr)
						   ? (&bgp_attr_get_srv6_l3service(bpi->attr)->sid)
						   : (&bgp_attr_get_srv6_vpn(bpi->attr)->sid);
		vty_out(vty, " sid=%pI6", sid_tmp);

		if (bgp_attr_get_srv6_l3service(bpi->attr) && bgp_attr_get_srv6_l3service(bpi->attr)->loc_block_len != 0) {
			vty_out(vty, " sid_structure=[%d,%d,%d,%d]",
				bgp_attr_get_srv6_l3service(bpi->attr)->loc_block_len,
				bgp_attr_get_srv6_l3service(bpi->attr)->loc_node_len,
				bgp_attr_get_srv6_l3service(bpi->attr)->func_len,
				bgp_attr_get_srv6_l3service(bpi->attr)->arg_len);
		}
	}
