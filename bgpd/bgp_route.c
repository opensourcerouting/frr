		...
		struct bgp_route_evpn *evpn)
{
	...
	zlog_info("BLET BGP_UPDATE: %p", evpn);
	if (evpn)
		zlog_info("BLET BGP_UPDATE: %pIA", &evpn->gw_ip);
	...
	if (!soft_reconfig &&
		...
		if ((afi == AFI_L2VPN) && evpn) {
			zlog_info("BLET: %s/%pIA", __func__, &evpn->gw_ip);
		       ...
		       struct bgp_static *bgp_static, afi_t afi, safi_t safi)
{
	...
	if (afi == AFI_L2VPN) {
		if (bgp_static->gatewayIp.family == AF_INET) {
			...
			zlog_info("BLET: 1 %s/%p", __func__, bre);
			...
			memcpy(&bre->gw_ip.ipaddr_v6,
			       ...
			       IPV6_MAX_BYTELEN);
			bgp_attr_set_evpn_overlay(&attr, bre);
			zlog_info("BLET: 2 %s/%p", __func__, bre);
		}
			   ...
			   json_object *json_paths)
{
	...
	struct bgp_route_evpn *eo = bgp_attr_get_evpn_overlay(attr);
