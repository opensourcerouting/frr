            ...
            with open(aspa_file, "r") as fh:
                for row in csv.reader(fh):
                    if not row or row[0].strip().startswith("#"):
                        continue
                    self.aspa.append(
                        (int(row[0]), [int(p) for p in row[1:] if p.strip()])
                    )

    def get_aspa(self, serial: int = 0) -> List[Tuple[int, List[int]]]:
...
class RTRConnHandler(socketserver.BaseRequestHandler):
    ...
    def send_endofdata(self, serial: int):
        ...
        if self.PROTO_VERSION == 0:
            ...
            # Version 1 and 2 carry the intervals in the End of Data PDU.
            self.request.send(
                struct.pack(
                    "!BBHIIIII",
                    self.PROTO_VERSION,
    ...
    def send_aspa(self, customer_asn: int, provider_asns: List[int], flags: int):
        # ASPA PDU, RTR version 2.  Unlike most PDUs the two bytes after the
        # type are "flags" and a zero byte rather than a 16 bit reserved
        # field.  rtrlib rejects a withdrawal that carries providers.
        providers = list(provider_asns) if flags else []
        length = self.ASPA_HDR_LEN + 4 * len(providers)
        dbg(f"<ASPA customer={customer_asn} providers={providers} flags={flags}")
        self.request.send(
            struct.pack(
    ...
    def announce_aspa(self, customer_asn: int, provider_asns: List[int]):
    ...
    def handle_serial_query(self, buf: bytes, sess_id: int):
        ...
        for customer_asn, providers in self.server.db.get_aspa(serial):
