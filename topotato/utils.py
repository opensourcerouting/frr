...
def _json_compare(d1: Any, d2: Any, result: JSONCompareResult, path: str = ""):
    ...
    if isinstance(d2, JSONCompareDirective):
        raise JSONCompareDirectiveWrongSide("JSONCompareDirective seen on the 'actual data' side of a compare.")
    ...
    if isinstance(d1, dict) and isinstance(d2, dict):
        ...
        if len(d1) < len(d2) and not any(isinstance(x, JSONCompareIgnoreExtraListitems) for x in d2):
            result.add_error(f"Actual data has fewer elements than expected:\n ({len(d1)} < {len(d2)}) at {path}", d1, d2)
        for i, v2 in enumerate(d2):
            ...
            if isinstance(v2, JSONCompareDirective):
                if isinstance(v2, JSONCompareIgnoreExtraListitems):
                    break
                elif isinstance(v2, JSONCompareListKeyedDict):
                    keying = v2.keying
                    # Compare both lists by looking for matching items regardless of order
                    keyed_d1 = {tuple(str(d1_item[k]) for k in keying): d1_item for d1_item in v1}
                    keyed_d2 = {tuple(str(d2_item[k]) for k in keying): d2_item for d2_item in v2 if all(k in d2_item for k in keying)}
                    _json_compare(keyed_d1, keyed_d2, result, p)
                    continue
                else:
                    raise JSONCompareUnexpectedDirective(f"Unexpected JSONCompareDirective in list at {path}")
            else:
                _json_compare(v1, v2, result, p)

    else:
        if d1 != d2:
            result.add_error(f"Actual data mismatch at {path}.\nExpected: {d2}, Actual: {d1}", d1, d2)
