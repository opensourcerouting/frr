...
static uint16_t parse_port(const char *port_string, char *error, size_t error_size)
{
	...
	if (rv == 0 && errno == EINVAL) {
		if (error)
			snprintf(error, error_size, "invalid format: %s",
				 port_string);
		return 0;
	}
	...
	if ((rv <= 0 || rv >= 65535) || errno == ERANGE) {
		if (error)
			snprintf(error, error_size, "outside valid range: %s",
				 port_string);
		return 0;
	}
	...
	if (*nullbyte != 0) {
		if (error)
			snprintf(error, error_size, "unexpected ending: %s",
				 nullbyte);
		return 0;
	}
...
bool network_address_parse(const char *address_string,
			   ...
			   uint16_t default_port)
{
	...
	if (str_pos == NULL) {
		snprintf(address->error, sizeof(address->error),
			 "invalid address format: %s", address_string);
		return false;
	}
	...
	if (strcmp(type, "unix") == 0 || strcmp(type, "unixc") == 0) {
		...
		if (str_pos != NULL) {
			...
			*str_pos = 0;
			port = parse_port(str_pos + 1, port_error,
					  sizeof(port_error));
			if (port == 0) {
				snprintf(address->error, sizeof(address->error),
		...
		struct sockaddr_in6 *sin6 =
			(struct sockaddr_in6 *)&address->address;
		...
		if (*str_pos != '[') {
			snprintf(address->error, sizeof(address->error),
				 "invalid IPv6 format (expected '['): %s",
				 addr);
			return false;
		}
