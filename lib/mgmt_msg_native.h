 ...
 * changes staged in @source (e.g., a batch of changes sent while holding the
 * datastore locks for a config file load). For those senders a failed commit
 * means the staged changes must be dropped, otherwise they are left in @source
 * diverged from @target where they would poison every subsequent commit.
 * Transactional senders, which accumulate changes over multiple commits, leave
 * this clear so a failure keeps @source intact for the user to fix or abort.
 */
