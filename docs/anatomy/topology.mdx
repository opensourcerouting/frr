...
- Refer to the [`ascii-diagrams`](#ascii-diagrams) section, for ASCII.
- `topology` function name is **not** required, so it can be renamed to anything.
- Parameter `topo`is **required** to modify the topology. Refer to [`topoology modifier`](#topology-modifier).
...
`NOMLinked` contains all the utility functions for finding other elements on the network
...
`IPPrefixIfaceList` is a list that only contains IPNetwork/IPInterface items.
