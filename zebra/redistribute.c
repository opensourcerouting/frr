			       ...
			       int afi)
{
	...
	for (rn = route_top(table); rn; rn = srcdest_route_next(rn))
		RNODE_FOREACH_RE (rn, newre) {
			...
			if (type == ZEBRA_ROUTE_TABLE_DIRECT)
				redistribute_table_direct(client,
				     ...
				     struct zserv *client)
{
	 ...
	 * `table-direct`. By default all table IDs that don't belong to
	 * a VRF gets learnt as `VRF_DEFAULT`.
	 */
	if (re->vrf_id == VRF_DEFAULT && zvrf->table_id != re->table) {
		if (re->table &&
		    redist_table_direct_has_id(&client->mi_redist
							[afi]
